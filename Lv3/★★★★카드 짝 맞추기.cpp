...
void setCardPositions(vector<pair<int, int>>& path, int num) {//num이라는 번호의 카드를 board에서 뽑아낸다.
    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 4; j++)
            if (board[i][j] == num)
                path.push_back({ i, j });
...
pair<int, int> getCtrlMove(int d, int y, int x) {//컨트롤을 눌렀을 때의 움직인 결과 위치를 반환한다.
    if (d - 4 < 2) {//y축에 대한 움직임
        while (ok(y, x)) {
            y += dy[d-4];
            if (!ok(y, x))break;
