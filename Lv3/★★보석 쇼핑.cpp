...
vector<int> solution(vector<string> gems) {
    ...
    int Min = pointer_e - pointer_s;
    answer[0] = pointer_s + 1;
    answer[1] = pointer_e + 1;
    while (pointer_e < gems.size()) {
        string gemName = gems[pointer_s];
        mp[gemName]--; pointer_s++;
        if (mp[gemName] == 0) {
            pointer_e++;
            for (; pointer_e < gems.size(); pointer_e++) {
                mp[gems[pointer_e]]++;

                if (gemName == gems[pointer_e])
                    break;

            }
            if (pointer_e == gems.size())break;
        }if (Min > pointer_e - pointer_s) {
            answer[0] = pointer_s + 1;
            answer[1] = pointer_e + 1;
            Min = pointer_e - pointer_s;
        }
    }
